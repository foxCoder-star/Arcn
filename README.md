...
Commit it:
bashcd /Users/davidmishael/Arcn
git add README.md
git commit -m "docs: update README to reflect current state"
git pushYou said: bro just give me in a format i can just paste it inteo the text editor  in the github websitebro just give me in a format i can just paste it inteo the text editor  in the github website6:15 PMClaude responded: ArcnArcn
A modular AI assistant built from the ground up in Python. Not a wrapper around an API — every component is understood, designed, and built deliberately.
Arcn listens to what you say, understands what you mean, and does it. It runs entirely on your machine.
...
Roadmap
 ...
 NLP — intent classification, entity extraction, multi-turn context
 Command Center — orchestration, routing, state management
 Tools — real Mac actions across 20 intent categories
 Speech — voice input via Whisper, voice output via Edge TTS
 Weather — live weather via OpenWeatherMap API
 Weather — natural language summary + Fahrenheit/Celsius support
 create_reminder — Mac Reminders app via osascript
 send_message — iMessage via phone number mapping
 Memory — persistent context across sessions with vector retrieval
...
Setup
Requirements: Python 3.12, macOS (Mac-first, cross-platform support planned)
bashgit clone https://github.com/foxCoder-star/Arcn.git
cd Arcn
NLP module:
bashcd nlp
python3 -m venv venv
...
deactivate
Command Center:
bashcd ../command_center
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
deactivate
Tools:
bashcd ../tools_assistant
python3 -m venv venv
source venv/bin/activate
pip install -r requirements.txt
deactivate
